...
from .modeling_outputs import BaseModelOutput, MultipleChoiceModelOutput, SequenceClassifierOutput
...
class DebertaForMultipleChoiceAndUnanswerable(DebertaPreTrainedModel):
    def __init__(self, config, mcq, answerable):
        super().__init__(config)
        ...
        self.classifier = torch.nn.Linear(output_dim, self.mcq)
        self.answerable_classifier = torch.nn.Linear(output_dim,self.answerable)
        self.init_weights()
    ...
    def forward(
        ...
        pooled_output = self.dropout(pooled_output)
        logits = self.classifier(pooled_output)
        logits_answerable = self.answerable_classifier(pooled_output)
        reshaped_logits = logits.view(-1, self.mcq)
        reshaped_answerable_logits = logits_answerable.view(-1,self.answerable)

        loss = None
        if labels is not None:
            loss_fct = CrossEntropyLoss()
            loss_mcq = loss_fct(reshaped_logits,labels)
            loss_anserwable = loss_fct(reshaped_answerable_logits, answerable)
            loss = loss_mcq + loss_anserwable

        if not return_dict:
            output = (reshaped_logits,) + (reshaped_answerable_logits,) + outputs[2:]
            return ((loss,) + output) if loss is not None else (logits, reshaped_answerable_logits) #output
...
class DebertaForMultipleChoiceAndReasoningTypes(DebertaPreTrainedModel):
    def __init__(self, config, reasoning_types):
        super().__init__(config)
        ...
        self.reasoning_classifier = torch.nn.Linear(output_dim,self.reasoning_types)
        self.init_weights()
    ...
    def forward(
        ...
        pooled_output = self.pooler(encoder_layer)
        pooled_output = self.dropout(pooled_output)
        logits = self.reasoning_classifier(pooled_output)
        reshaped_logits = logits.view(-1, self.reasoning_types)

        loss = None
        if labels is not None:
            loss_fct = CrossEntropyLoss()
            loss = loss_fct(reshaped_logits, reasoning_labels)
